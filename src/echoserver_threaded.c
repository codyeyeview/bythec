...
void remote_connection(){;
    ...
    base = event_base_new();
    printf("connecting...\n");
    conn = evhttp_connection_base_new(base, NULL, "54.173.82.250", 15120);
...
void buffered_on_read(struct bufferevent *bev, void *arg) {
	...
	while ((nbytes = EVBUFFER_LENGTH(bev->input)) > 0) {
		...
		evbuffer_add(client->output_buffer, p, 3);
	}
...
void on_accept(int fd, short ev, void *arg) {
	 ...
	 * the events, in this case the clients socket. 15120
	 *
	 ...
	 * object here.
	 */
	if ((client->buf_ev = bufferevent_new(client_fd, buffered_on_read,
			buffered_on_write, buffered_on_error, client)) == NULL) {
...
int runServer(void) {
	...
	sigset_t sigset;
	sigemptyset(&sigset);
	struct sigaction siginfo = { .sa_handler = sighandler, .sa_mask = sigset,
			.sa_flags = SA_RESTART, };
	...
	event_set(&ev_accept, listenfd, EV_READ | EV_PERSIST, on_accept,
			(void *) &workqueue);
